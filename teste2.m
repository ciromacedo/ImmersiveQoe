...
%Rosana - parte do RDk
Rd = [];  %matriz de taxa de downlink 
Rd = zeros(1,3);
for k = 1:3
    Rd(k) = randi([10, 42]);
end
for i = 1:3
    Rd(i) =  (max (Rd)- min (Rd))/10;
end
...
usernum = 3;% change to try different users (1~30)
...
RFINAL(u,2) = Rd(usernum)* sum(uoal.*log(PnkR./PthR));
% sum(PnkR)
...
RFINAL(u,3) = Rd(usernum) * sum(uoal.*log(PnkR./PthR));
% sum(PnkR)
...
FINAL(u,5) = Rd(usernum) * sum(uoal.*log(PnkR./PthR));
